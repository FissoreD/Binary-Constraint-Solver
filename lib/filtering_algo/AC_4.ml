module AC_4 : Arc_consistency.Arc_consistency = struct
  ...
  type 'a data_struct = (string, 'a cell_type) Hashtbl.t
  ...
  let initialization ?(print = false) (graph : 'a Constraint.graph) =
    ...
    Hashtbl.iter
      (fun _ (d : 'a DoublyLinkedList.t) ->
        DLL.iter
          (fun v ->
            Hashtbl.add data_struct
              (Arc_consistency.make_name v)
              (v, DLL.empty ""))
          d)
      graph.domains;
    let add_new_pair elt =
      Hashtbl.find data_struct (Arc_consistency.make_name elt)
    in
    Hashtbl.iter
      (fun _ ((a : string DLL.dll_node), (b : string DLL.dll_node)) ->
        if a.is_in && b.is_in then (
          let x, y = (add_new_pair a, add_new_pair b) in
          let last_x = DLL.append { value = b; assoc = None } (snd x) in
          let last_y = DLL.append { value = a; assoc = None } (snd y) in
  ...
  let revise (input : 'a DLL.dll_node) (data_struct : 'a data_struct) =
    ...
    match Hashtbl.find_opt data_struct (Arc_consistency.make_name input) with
